...
\item [\refstepcounter{mnum} \mthemnum \label{mObject}:] Object Module
...
\item [\refstepcounter{mnum} \mthemnum \label{mShade}:] Shader Module
...
\item [\refstepcounter{mnum} \mthemnum \label{mVectors}:] Vector Math Module
\end{description}
...
& JSON Module\\ %I need the JSON module to parse and record inputs and output 
%to files
...
\begin{description}
	\item[Secrets:]The algorithms to perform vector calculus.
	\item[Services:]Calculates normalized versions of vectors, unit vectors, 
	and basic vector operations (addition, subtraction, dot and 
	cross products).
	\item[Implemented By:] ---
\end{description}
